...
def set_product_cost(a):
    ...
    if cost_before_discount:
        ...
        with conn() as cursor:
            cursor.execute("update product set (cost, timestamp_) = (%s, %s) where id = %s returning name, cost, timestamp_", (cost, timestamp_, a[0]))
            result = cursor.fetchall()
        cf.pretty_(['name', 'cost', 'timestamp_'], result)
