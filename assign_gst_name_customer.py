...
for a in result:
    gst_name = a[0].strip()
    gst_number = a[2].strip()
    print(a)
    r = cf.prompt_("Enter estimate name: ", without)
    sq = "update customer set (gst_name, gst_number) = (%s, %s) where nickname = %s"
    print("gst_name: {}\n gst_number: {}\n nickname:{}\n".format(gst_name, gst_number, r))
    confirm_ = cf.prompt_("Do you want to make the above entry?: ", ['y', 'n',  'stop'], unique_="existing")
    if confirm_ == 'stop':
        break
    elif confirm_ == 'n':
        continue
        ...
        with conn() as cursor:
            cursor.execute(sq, (gst_name, gst_number,r ))
